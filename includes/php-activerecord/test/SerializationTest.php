...
class SerializationTest extends DatabaseTest
{
	...
	{
		$this->assert_has_keys('name','author', $this->_a(array('only' => 'name', 'includes' => 'author')));
	...
	{
		$a = $this->_a(array('includes' => array('author')));
	...
	{
		$a = $this->_a(
			array('includes' => array('events' => array('except' => 'title', 'includes' => array('host' => array('only' => 'id'))))),
			Host::find(4));
	...
	{
		$xml = Host::find(4)->to_xml(array('includes' => 'events'));

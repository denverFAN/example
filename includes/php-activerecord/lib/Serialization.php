 ...
 * <li><b>includes:</b> a string or array of associated models to includes in the final serialized product.</li>
 ...
 *
 * <code>
 * # includes the attributes id and name
 * # run $model->encoded_description() and includes its return value
 * # includes the comments association
 * # includes posts association with its own options (nested)
 * $model->to_json(array(
 *   'only' => array('id','name', 'encoded_description'),
 *   'methods' => array('encoded_description'),
 *   'includes' => array('comments', 'posts' => array('only' => 'id'))
 * ));
 *
...
abstract class Serialization
{
	...
	{
		if (isset($this->options['includes']))
		{
			$this->options_to_a('includes');

			$serializer_class = get_class($this);

			foreach ($this->options['includes'] as $association => $options)
			{
				if (!is_array($options))
				{
					$association = $options;
